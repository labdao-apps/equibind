...
def parse_arguments(arglist = None):
    ...
    p.add_argument('--config', type=argparse.FileType(mode='r'), default= False) #'configs_clean/inference.yml'
    ...
    p.add_argument('-r','--rec_path', type=str, help='path to a pdb file for which you want to run inference')
    p.add_argument('-l','--lig_path', type=str, help='path to a directory with ligands for which you want to run inference')
    ...
    p.add_argument('--use_rdkit_coords', action="store_true", default = True,
    ...
    p.add_argument('--no_use_rdkit_coords', action="store_false", dest = "use_rdkit_coords",
                   help='override the rkdit usage behavior of the used model')
    p.add_argument('--run_dirs', type = list, default = ['flexible_self_docking'])
    p.add_argument('--test_names', type = str, default = 'timesplit_test')
    p.add_argument('--multi_ligand', type = bool, default = False, help = 'If screening multiple ligands in a single SDF file select enter True')
...
def multi_lig_inference(args):
    ...
    for idx, name in enumerate(names):
        if not name.startswith('.'):
            print(f'\nProcessing {name} against all provided ligands')
            file_names = os.listdir(os.path.join(args.inference_path, name))
            rec_name = [i for i in file_names if 'protein' in i][0]
            lig_names = [i for i in file_names if 'ligands' in i]
            rec_path = os.path.join(args.inference_path, name, rec_name)
            for lig_name in lig_names:
                if lig_names == []: raise ValueError(f'No ligand files found. The ligand file has to contain \'ligands\'.')
                
                for lig_name in lig_names:
                    if not lig_name.endswith('.sdf'): raise ValueError('Multi-Ligand can only accept files in the sdf file format.')
                    if not os.path.exists(os.path.join(args.inference_path, name, lig_name)):
                        raise ValueError(f'Path does not exist: {os.path.join(args.inference_path, name, lig_name)}')
                    print(f'Trying to load {os.path.join(args.inference_path, name, lig_name)}')
                ligs = Chem.SDMolSupplier(os.path.join(args.inference_path, name, lig_name))
                passed = []
                failed =[]
                print(f'Docking the receptor {os.path.join(args.inference_path, name, rec_name)}\nTo the ligands in {lig_names}')
                rec, rec_coords, c_alpha_coords, n_coords, c_coords = get_receptor_inference(rec_path)
                rec_graph = get_rec_graph(rec, rec_coords, c_alpha_coords, n_coords, c_coords,
                                      use_rec_atoms=dp['use_rec_atoms'], rec_radius=dp['rec_graph_radius'],
                                      surface_max_neighbors=dp['surface_max_neighbors'],
...
if __name__ == '__main__':
    ...
    '''
    print(args.inference_path)
    print(args.output_directory != None)
    print('\n')
  
    names = os.listdir(args.inference_path)
    ...
    for idx, name in enumerate(names):
        if not name.startswith('.'):
            ...
            #print(file_names)
            rec_name = [i for i in file_names if 'protein' in i][0]
            lig_name = [i for i in file_names if 'ligand' in i]
            print(rec_name, lig_name)
