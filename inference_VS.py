...
def inference_from_files(args):
    ...
    use_rdkit_coords = args.use_rdkit_coords if args.use_rdkit_coords != None else args.dataset_params[
        'use_rdkit_coords']
    file_paths = glob.glob(args.lig_path+'/*') if (args.inference_path != None) else tqdm(read_strings_from_txt('data/timesplit_test'))
    rec_path = args.rec_path
    if file_paths == []: raise ValueError(f'The ligand directory seems to be empty!')
    
    failed_list = []
    
    for idx, lig_path in enumerate(file_paths):
        
        lig_name = os.path.basename(lig_path).split('.')[0] 
        print(f'\nProcessing {lig_name}: complex {idx + 1} of {len(file_paths)}')
        lig = read_molecule(lig_path, sanitize=True)

        if lig == None: 
          print(f'This ligand file could not be read: {lig_name}')
          failed_list.append(lig_name)
          continue
        
        print(f'Docking the receptor to the ligand {lig_name}')
        ...
        lig_graph = get_lig_graph_revised(lig, lig_name, max_neighbors=dp['lig_max_neighbors'],
        ...
        with torch.no_grad():
            ...
            ligs_coords_pred_untuned, ligs_keypts, recs_keypts, rotations, translations, geom_reg_loss = model(
                lig_graph.to(device), rec_graph.to(device), geometry_graph, complex_names=[lig_name], epoch=0)
            ...
            if args.run_corrections:
                ...
                if args.output_directory:
                    ...
                    print(f'Writing prediction to {args.output_directory}/{lig_name}_EB.sdf')
                    with open(f'{args.output_directory}/{lig_name}_EB.sdf', "w") as newfile:
                            newfile.write(block_optimized)
            all_names.append(lig_name)
        ...
        torch.save(results, path)
    with open(f'{args.output_directory}/failed_compounds.txt', "w") as newfile:
        print('writing failed_compounds.txt')
        for compound in failed_list:
        	newfile.write(compound + '\n')
